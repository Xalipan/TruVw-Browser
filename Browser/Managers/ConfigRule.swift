...
struct RuleMatch: Codable {
    ...
    func matches(_ url: URL) -> Bool {
        ...
        if let query = queryContains {
            // Normalise both sides: decode percent-encoding, replace + with space, lowercase
            let normalize: (String) -> String = {
                $0.replacingOccurrences(of: "+", with: " ")
                  .removingPercentEncoding ?? $0
            }
            let urlQuery  = normalize(url.query ?? "").lowercased()
            let matchTerm = normalize(query).lowercased()
            guard urlQuery.contains(matchTerm) else { return false }
        }
