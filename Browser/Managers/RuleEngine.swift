...
class RuleEngine {
    ...
    func apply(rules: [ConfigRule], to webView: WKWebView, url: URL) {
        // Fire immediately
        applyOnce(rules: rules, to: webView, url: url)
        // Fire again after a short delay to catch dynamically rendered content
        // (e.g. Google search results which load after didFinish fires)
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak webView] in
            guard let webView = webView else { return }
            self.applyOnce(rules: rules, to: webView, url: url)
        }
    }

    private func applyOnce(rules: [ConfigRule], to webView: WKWebView, url: URL) {
        for rule in rules {
